...
export class WebhookDispatchAdapter implements DispatchAdapter {
  name = 'webhook';
  ...
  async execute(input: DispatchAdapterExecutionInput): Promise<DispatchAdapterExecutionResult> {
    ...
    pushLog(logs, 'info', `webhook posting run ${input.runId} to ${webhookUrl}`);
    ...
    pushLog(logs, response.ok ? 'info' : 'error', `webhook response status: ${response.status}`);
    ...
    const maxWaitMs = clampInt(readNumber(input.context?.webhook_max_wait_ms), DEFAULT_MAX_WAIT_MS, 1_000, 15 * 60_000);
    const startedAt = Date.now();
    pushLog(logs, 'info', `webhook polling status from ${pollUrl}`);

    while (Date.now() - startedAt < maxWaitMs) {
      if (input.isCancelled?.()) {
        pushLog(logs, 'warn', 'webhook run cancelled while polling');
        return {
          status: 'cancelled',
