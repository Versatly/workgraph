...
function touchAgentLastSeen(workspacePath: string, actor: string): void {
  const safeActor = actor.trim();
  if (!safeActor) return;
  const relPath = `agents/${safeActor.toLowerCase().replace(/[^a-z0-9_-]+/g, '-')}.md`;
  ...
  if (!existing) {
    try {
      store.create(
        ...
        `# ${safeActor}\n\nAutogenerated by thread lifecycle heartbeat.\n`,
        'system',
        { pathOverride: relPath },
      );
    } catch {
      // Best-effort visibility update should never block thread lifecycle operations.
    }
    return;
  }
  try {
